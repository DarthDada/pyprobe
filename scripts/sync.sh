...
# falls back to installing the package (editable) plus the dev tools (pytest,
# pytest-cov, watchfiles) via pip.
#
# Usage:
#   scripts/sync.sh
set -e
...
if have_uv; then
    ...
    python3 -m pip install pytest pytest-cov watchfiles
fi
