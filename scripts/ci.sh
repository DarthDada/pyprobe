...
stages=("$@")
if [ $# -eq 0 ] || [ "$1" = "all" ]; then
    stages=(sync gen-offsets lint test smoke)
elif [ "$1" = "full" ]; then
    stages=(sync gen-offsets build-c lint test smoke)
fi
